...
<div class="card-grid">
{% for post in site.posts %}<article class="card"><p class="card-meta">{{ post.date | date: "%B %-d, %Y" }}</p><h2><a href="{{ post.url | relative_url }}">{{ post.title }}</a></h2><p>{{ post.excerpt }}</p></article>{% endfor %}
</div>
