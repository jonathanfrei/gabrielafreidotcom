...
{% for event in sorted_events %}{% assign event_timestamp = event.event_date | date: "%s" | plus: 0 %}{% if event_timestamp >= now_timestamp %}{% assign upcoming_count = upcoming_count | plus: 1 %}
...
<h2>Past events</h2>
<div class="event-list">
{% for event in sorted_events reversed %}{% assign event_timestamp = event.event_date | date: "%s" | plus: 0 %}{% if event_timestamp < now_timestamp %}<article class="event-row"><time class="event-date" datetime="{{ event.event_date | date_to_xmlschema }}">{{ event.event_date | date: "%b" }}<strong>{{ event.event_date | date: "%-d" }}</strong></time><div><h3><a href="{{ event.url | relative_url }}">{{ event.title }}</a></h3><p>{{ event.venue }} · {{ event.location }}</p></div></article>{% endif %}{% endfor %}
</div>
